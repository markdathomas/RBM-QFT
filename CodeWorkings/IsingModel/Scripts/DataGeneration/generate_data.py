...
def generate_data_folder(run_parameters, new_folder_absolute_path, folder_name, array_name, topdir="../../"):
    ...
    Parameters:
    - run_parameters (numpy.ndarray): An array containing various learning parameters.
    - new_folder_absolute_path (str): The absolute path to the new folder.
    - folder_name (str): The name of the new folder.
    - array_name (str): The name to be given to the saved array.
    - topdir (str, optional): The path to the uppermost directory needed to view the full project.

    Returns:
    - str: The absolute path of the saved NumPy array.
    """
    ...
    # Check if the folder already exists; if not, make the directory:
...
def run(step_size_list, m_visible, n_hidden, alpha_list, k_steps_list, batch_size):
    ...
    Data is saved as "raw_data.npy" in the Data folder, with specified m, n, alpha, and S labeled
    along with the date of generation.

    Returns the location of the saved data.

    The run parameters are:
    - step_size_list: List containing the number of steps in each learning phase, e.g., [100, 100, 100].
    ...
    # Create an array with run parameters
