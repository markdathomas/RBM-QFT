...
def data_exp_wij(v, theta, arg):
    """
    Function to be averaged over, an argument of E_{data} as 
    in equation 13. It replaces p(hi=1) with 2p(hi=1)-1, 
    as spins are measured in the (-1,1) basis, not (0,1).
    
    Returns the associated term for the argument of the E_{data} sum.
    """
    i, j = arg
...
def data_exp_bj(v, theta, arg):
    """
    Function to be averaged over, as in equation 17. 
    This time the calculation does not depend on the choice of 
    basis for spin, so no edit is made to this term.
    """
    j = arg
    if type(v) == int:
        return 0
...
def data_exp_ci(v, theta, arg):
    """
    Function to be averaged over, an argument of E_{data} as 
    in equation 19. It replaces p(hi=1) with 2p(hi=1)-1, 
    as spins are measured in the (-1,1) basis, not (0,1).
    """
    i = arg
...
def model_exp_wij(v, theta, arg):
    ...
    """
    i, j = arg
    prob_term = prob_hi_is_1(i, v, theta)  # Converted into the new basis.
    vj_term = v[j]
...
def model_exp_ci(v, theta, arg):
    """
    Function to be averaged over, an argument of E_{model} as 
    in equation 19. It replaces p(hi=1) with 2p(hi=1)-1, 
    as spins are measured in the (-1,1) basis, not (0,1).
    """
    i = arg
...
def e_data_wij(S, i, j, theta, Z):
    ...
    Be careful that data_exp_wij is in the right basis for how things
...
def e_data_bj(S, j, theta):
    ...
    of the choice of basis for the Ising-model spins, so
